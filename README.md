...
The public API is declared in `csa.h`. The main operations are:
...
| File | Role |
| --- | --- |
| `driver.c` | Runs assertions for core CSA behaviour and extension behaviour |
| `fibmemo.c` | Uses the CSA as a memo table for recursive Fibonacci |
| `isfactorial.c` | Stores factorial numbers in a CSA and prints them later |
| `sieve.c` | Uses CSA deletion and iteration to implement the Sieve of Eratosthenes |
